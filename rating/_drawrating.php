...
$tense=($count==1) ? "voto" : "voti"; //plural form votes/vote
...
if ($static == 'static') {
		...
		$static_rater[] .= '</ul>';
		$static_rater[] .= '<p class="static">'.$id.'. Rating: <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' totali) <em>This is \'static\'.</em></p>';
		$static_rater[] .= '</div>';
		$static_rater[] .= '</div>'."\n\n";
      ...
      $rater.='>Voto: <strong> '.$rating1.'</strong>/'.$units.' ('.$count.' '.$tense.' in totale)';
      $rater.='  </p>';
      $rater.='</div>';
      $rater.='</div>';
      return $rater;
